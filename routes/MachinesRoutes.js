...
LoaderRoutes.post('/get_sensors', (req, resw) => { // get list of all unique sensor ids that sent reports
    res.json({ message: "create this route" })
})


LoaderRoutes.post('/get_sensor_data', async (req, res) => {// returns latest 10 reports of a sensor or by day
    if (req.body.sensor_id === undefined ) { return res.json({ error: "missing params" }) }
    if (req.body.specific_date === undefined ) { return res.json({ error: "missing params" }) }
